...
The local web experience lets a person select one JPEG, PNG, or WebP portrait up to 10 MiB, submit it to the API, and view a completed mock evidence report. The API independently validates multipart `portrait` content, stores accepted bytes privately for at most 24 hours, then runs local face-quality gates in the background. `GET /v1/jobs/{job_id}` exposes a report only after processing completes; it contains safe decoded-image metadata (format, dimensions, and metadata presence) plus quality findings, never image bytes, embedded metadata values, a client filename, or a facial embedding. The mock report is always `inconclusive`: C2PA/provenance verification and synthetic-portrait detector scores arrive in later milestones, and metadata presence or absence is never evidence of authenticity. See [the API contract](docs/api-contract.md) and [face-quality policy](docs/face-quality.md).
